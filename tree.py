...
class BinaryTree:
    ...
    def proximity_search_contains(self, value: Any):
        """
        Busca y muestra los nodos cuyo valor contiene la subcadena 'value'.
        Funciona solo si los valores de los nodos son strings.
        """
        def __search(root, value):
            if root is not None:
                if isinstance(root.value, str) and value in root.value:
                    print(root.value, root.other_values)
                __search(root.left, value)
                __search(root.right, value)
